...
\begin{definicion*}{Métrica $\rho$}{}
  ...
  \begin{itemize}
    ...
    \[
    \begin{array}{lll}
      1. & \left( \rho(x,y) > 0 \Leftrightarrow x \neq y \right) \wedge \left( \rho(x,y) = 0 \Leftrightarrow x = y \right) & \forall_{x \in X}\forall_{y \in X} \\
      2. & \rho(x,y) = \rho(y,x) & \forall_{x \in X}\forall_{y \in X} \\
      3. & \rho(x,y) \leq \rho(x,z) + \rho(z,y) & \forall_{x \in X}\forall_{y \in X}\forall_{z \in X}
    \end{array}
    \]
    \item Ejemplos
    \begin{enumerate}
